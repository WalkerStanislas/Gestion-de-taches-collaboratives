...
class Task:
    def __init__(self, task=None, category=None, date_added=datetime.datetime.now().isoformat(),
    ...
    def save(self):
        ...
        with conn:
            cursor.execute('INSERT INTO taches VALUES (:task, :category, :date_added, :date_completed, :status, :position, :user_id)',
                        {'task':self.task, 'category':self.category, 'date_added': self.date_added, 'date_completed':self.date_completed, 'status':self.status,'position':self.position, 'user_id':self.id_User})
    ...
    def update(self):
        ...
        with conn:
            cursor.execute('UPDATE taches SET task = :task, category = :category WHERE position = :position',
                            {'position':self.position, 'task':self.task, 'category':self.category})

    def delete(self):
        """Supprimer une taches en fonction de sa position"""
        count = self.count()
        with conn:
            cursor.execute("DELETE FROM taches WHERE position=:position", {"position": self.position})
    ...
    def complete_task(self):
        with conn:
            cursor.execute('UPDATE taches SET status = 3, date_completed = :date_completed WHERE position = :position',
                        {'position':self.position, 'date_completed':datetime.datetime.now().isoformat()})
